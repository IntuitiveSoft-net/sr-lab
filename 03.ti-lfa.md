...
:Note: Adj-SID may differ in your lab
:keyboard: xrd-1
```bash
...
:note: Notice the utilisation of adjacency SID `24003` to force traffic through the high IGP cost link xrd-5/xrd-6, once the traffic is in the `Q space` it's safe to reach `xrd-3` with a single label `16003`.