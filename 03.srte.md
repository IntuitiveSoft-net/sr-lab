...
<details><summary><b>SR-TE configurations snippets</b></summary>
...
Visualize the local traffic engineering topology.
...
xrd-1 doesn't seem to know any topology. To be able to locally compute a label stack path, a router need to have a link state database.

### Task. 2 Configure the link state database distribution
...
Once the distribution of the main IGP process into the link state DB you should now be able to visualize the traffic engineering topology.
...
* endpoint :  It indicates the destination of the policy.  The endpoint
   is specified as an IPv4 or IPv6 address and SHOULD resolve to a
   unique node in the domain.
...
If you display the mpls database you should be able to find it.
...
### Task. 4 Static Steering

For now the policy is just signalized in the IGP. There's no routing information set in the RIB. It's a policy not a tunnel.
...
Segment-List is associated with a weight for weighted load balancing. The default weight is 1.
...
criterion is the reachability of its constituent SIDs. 
...
Add a new dynamic candidate path with an higher preference.
...
Visualize the state of the policy. 
